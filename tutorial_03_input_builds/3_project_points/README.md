**Project points** hold the coordinates and SAM configuration selection for each target location you wish to reV. These points must be taken from the meta data of the resource data for the technology that is being run (e.g., NSRDB for PV, WTK for wind).  The example code in this folder is using a subset of the wind toolkit points and subsetting them to contain points that are only found in Rhode Island.
